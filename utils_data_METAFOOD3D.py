...
###########################################################
class DepthToRGB:
    """自定义Transform，将深度图转换为3通道"""
    def __call__(self, img):
        # img 预计是PIL Image或Tensor
        if isinstance(img, torch.Tensor):
            # 如果已经是Tensor，假设形状是 [C, H, W] 或 [H, W]
            if img.dim() == 2:
                img = img.unsqueeze(0)  # [H, W] -> [1, H, W]
            rgb_tensor = img.repeat(3, 1, 1)  # 在通道维度复制3次 [3, H, W]
            return rgb_tensor
        else:
            # 如果是PIL Image，先转换为numpy数组或直接处理
            img_array = np.array(img)
            if len(img_array.shape) == 2:
                img_array = np.expand_dims(img_array, axis=-1) # [H, W] -> [H, W, 1]
            rgb_array = np.repeat(img_array, 3, axis=-1)
            return Image.fromarray(rgb_array.squeeze()) # 注意PIL Image需要处理3通道数组
##############################################################
...
#################################################################################
        ...
        train_transform = transforms.Compose([
            ...
            # 锐度调整，0.5%的概率，乘以2
            transforms.Lambda(lambda img: img if np.random.rand() > 0.005 else ImageEnhance.Sharpness(img).enhance(2)),
            # 自动对比度调整，10%的概率
            transforms.Lambda(lambda img: img if np.random.rand() > 0.1 else  ImageEnhance.Contrast(img).enhance(1.5)),
            transforms.ToTensor(),
            transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
        ])
        ...
        # 深度图像变换
        test_transform_D = transforms.Compose([
            transforms.Resize((550, 550)),
            transforms.CenterCrop((224,224)),
            DepthToRGB(), # 自定义转换
            transforms.ToTensor(),
        ...
        dataset_path = "/Datasets/MetaFood3D"  # 替换为实际数据集路径
        split_info_path = "/Datasets/MetaFood3D/split_info.json"  # 替换为实际数据集路径
        nutrition_file_path = "/Datasets/MetaFood3D/MetaFood3D_nutrition.xlsx"  # 替换为实际数据集路径

        train_dataset = Nutrition_RGBD(
            dataset_path=dataset_path,
            split_info_path=split_info_path,
            nutrition_file_path=nutrition_file_path,
            mode='train',
            transform=train_transform,
            transform_D=train_transform_D
        )
        
        test_dataset = Nutrition_RGBD(
            dataset_path=dataset_path,
            split_info_path=split_info_path,
            nutrition_file_path=nutrition_file_path,
            mode='test',
            transform=test_transform,
            transform_D=test_transform_D
        )
